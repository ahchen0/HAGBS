...
// Module constants
static const unsigned int MAX_PACKET = 256;  // bytes
...
/*void ax25_send_signed_char_arr(const int8_t *data_arr) {
  int i;
  for (i = 0; [i]; i++) {
    ax25_send_byte(data_arr[i]+'0');
  }
}*/

// NOTE: added in PBR params to save memory by making PBV copies
uint8_t getDigit2(const int32_t &num, const int32_t &n){
  int32_t r = pow(10, n);
  int32_t x = (num / r) % 10;
  return x;
}

void ax25_send_int(int32_t num, int digits){
  if(num < 0){
    ax25_send_byte('-');
    num *= -1;
  }
  for(int i = digits - 1; i >= 0; i--){
    ax25_send_byte(getDigit2(num, i) + '0');
  }
}
