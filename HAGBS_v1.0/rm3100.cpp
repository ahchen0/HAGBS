 ...
 *  @return     0 if successful, 1 otherwise.
 */
bool setCMMdatarate ( uint8_t conf ) {
 ...
 *  @return     0 if successful, 1 otherwise.
 */
bool continuousModeConfig ( uint8_t conf ) {
...
bool getRM3100Status ( void ) {
    uint8_t data[1];
    uint8_t to_reg = STE_ON | BW_11 | BP_11;
    uint8_t* ptr;
 ...
 */
uint8_t getRM3100revision ( void ) {
